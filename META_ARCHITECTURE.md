...
>
> **Last updated:** 2026-08-27 — a scheduled `promo-capture` lane joins the routines layer (twice-weekly read-only snapshot of repo traffic/stars before the 14-day API window expires, sentinel-tracked); the public-repo portfolio grew by three (a template starter, a dead-man's-switch utility, a redaction-check Action), each wired into the promotion-metrics, placement-health and audit surfaces at creation.
...
| `promo-capture` | Twice weekly (catch-up on missed slots) | Read-only snapshot of GitHub traffic, stars and referrers for the public repos, via the promotion-metrics tool's `capture` subcommand. Exists because the traffic API holds only a rolling 14-day window — an uncaptured fortnight is lost permanently. Capture is the sole scheduled part; reporting stays manual-fire by standing constraint. Writes a success sentinel consumed by the dead-man's-switch freshness tracker (Pattern 3). |
