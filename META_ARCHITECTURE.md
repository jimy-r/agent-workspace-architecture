...
| `overnight` | **Added 2026-09-24.** Harvests the task board, the audit ledger and open plan items, curates a no-input queue with the operator by menu, then drains it unattended behind a usage gate that pauses near the 5-hour cap and resumes after the reset. Outward steps stop at feature-branch pushes and draft PRs on the operator's own repositories. |
