...
|---|---|---|---|---|---|---|
| **Audit** | Weekly + on-demand workspace self-review: configs, hooks, security envelope, plugin/MCP bloat, external opportunities, memory drift. Findings → ledger → tiered findings in the task list. | `audit`, `audit-second-opinion` | — | `audit.bat`, `audit-second-opinion.bat`, `audit_ledger.py`, `audit_cost.py`, `security/check_task_freshness.py`, `ghost_token_counter.py` (shared with *Context Budget*), `tests/audit_canaries/` | `<workspace>/scripts/_state/audit_findings.jsonl`, `audit_cost.jsonl`, `ghost_tokens.db`, `tasks/scheduled-logs/upgrade-audit_*.log` | `<workspace>/.claude/agents/audit.md` |
| **Heartbeat** | Cron-driven (2h) project manager. Classify-then-act on the task + questions files; build `has-default` tasks in sandbox; lodge to review queue; log rejections as ADRs. Container-isolated variant under user-paced observation. | `heartbeat` | `review-queue` | `<workspace>/scripts/heartbeat/*` (classify_task, create_staging, check_rejections, idle_observations, host_reviewer, anthropic_proxy, observe_cycles) | `<workspace>/tasks/HEARTBEAT_REVIEWS.md`, `HEARTBEAT_REJECTIONS.md`, `tasks/heartbeat-sandbox/`, `tasks/scheduled-logs/heartbeat-monitor_*.log` | `<workspace>/tasks/HEARTBEAT.md`, project-level `OBSERVATION.md` runbook |
...
| **Roles** | Pure persona library (17 canonical roles) + project bindings under `.claude/agents/`. Composition: thin binding `@`-imports canonical role + project `CONTEXT.md`. Validator runs every heartbeat cycle. | 17 canonical + project bindings | `role-pressure-test` | `<workspace>/roles/_validate.py` | — | `<workspace>/roles/README.md`, `<workspace>/roles/_template.md` |
| **Memory** | User-global file-based memory: `MEMORY.md` index + topic files (`user_*`, `feedback_*`, `project_*`, `reference_*`) + `episodes/`. Weekly consolidation. Four-op discipline per fact. | — | `consolidate-memory` (scheduled-task) | `<workspace>/scripts/memory_lint.py` | `<home>/.claude/projects/<workspace-id>/memory/*` | `<home>/.claude/scheduled-tasks/consolidate-memory/SKILL.md`, user-global `CLAUDE.md § Memory hygiene` |
| **Security envelope** | Multi-layer file/command protection: PreToolUse hooks (Edit/Write + Bash), command-safety plugin (interactive CLI only), PreCompact transcript backup, `autoMode.hard_deny`. Credential discipline (password-manager + `.env` exception). | — | — | `<workspace>/scripts/security/check_bash_command.py`, `check_file_protection.py`, `precompact_backup.py` | hook execution log, `<workspace>/tasks/transcript-backups/` | META_ARCH § Hooks + § File protection |
...
| **Session workflow** | Skills that manage the session experience start-to-finish: orientation at start, terse-mode mid-session, checkpoints to bridge compactions, task readouts, close-out at end. `<workspace>/tasks/checkpoints/` is the cross-session continuity store. | — | `orient`, `wrap`, `tasks`, `context-save`, `context-restore`, `terse-mode` | — | `<workspace>/tasks/checkpoints/YYYY-MM-DD_HHMM_<slug>.md` | each skill's `SKILL.md` |
...
- `ghost_token_counter.py` — *Audit* (Phase 1 baseline) + *Context Budget* (not yet a module).
- `<workspace>/scripts/_state/audit_findings.jsonl` — *Audit* writes; *Heartbeat* could read in future for adaptive prioritisation.
- `<workspace>/scripts/security/*` — *Security envelope* owns; *Heartbeat* and *Audit* both consume.
...
- *Context Budget* — `ghost_token_counter.py` + audit Phase 1 trend + potential `claudemd-audit` integration.
- *Voice / Home Integration* — voice-channel MCP + a home-voice-interface project + a voice-booking-agent project.
- *Containerised execution* — Heartbeat is the first instance; sidecar pattern + check-mounts + anthropic-proxy generalise.
