...
| **Heartbeat** | Cron-driven (2h) project manager, **gated since 2026-06-10**: a Stage-0 wrapper preflight (`preflight_gate.py`) skips the LLM entirely when the watched task files are unchanged, the deterministic scans pass, and an agent cycle ran <24h ago; act-stage runs the work-tier model at max effort. Classify-then-act on the task + questions files; build `has-default` tasks in sandbox; lodge to review queue; log rejections as ADRs. Container-isolated variant parked (the agent CLI's subscription auth needs the desktop app's IPC; revisit with a provisioned API key). | `heartbeat` | `review-queue` | `<workspace>/scripts/heartbeat/*` (preflight_gate, classify_task, create_staging, check_rejections, idle_observations, host_reviewer, anthropic_proxy, observe_cycles) | `<workspace>/tasks/HEARTBEAT_REVIEWS.md`, `HEARTBEAT_REJECTIONS.md`, `tasks/heartbeat-sandbox/`, `scripts/_state/heartbeat_gate.json`, `tasks/scheduled-logs/heartbeat-monitor_*.log` | `<workspace>/tasks/HEARTBEAT.md`, project-level `OBSERVATION.md` runbook |
...
- **`token-optimization`** / `context-engineering` — **Token Budget** (owner) · *Audit* (ghost-token baseline + the over-median spend-trend rule) · *Heartbeat* (Stage-0 preflight gate = the flagship spend-avoidance control) · *Session workflow* (terse-mode, checkpoints that cut re-load cost). The most cross-cutting tag in the workspace.
- `safety` — **Security envelope** (owner) · *Heartbeat* (sandboxed builds + review gate) · *Audit* (security sweep).
- `agent-memory` — **Memory** (owner) · *Audit* (retrospective + semantic-drift check).
- `autonomy` — **Heartbeat** · *Brief* · *Inbox* (scheduled / approval-gated).
...
- `<workspace>/scripts/heartbeat/preflight_gate.py` — *Heartbeat* owns; *Token Budget* claims it as its flagship spend-avoidance control.
...
- *Containerised execution* — Heartbeat is the first instance; sidecar pattern + check-mounts + anthropic-proxy generalise. Currently parked (subscription auth needs the desktop app's IPC; see the Heartbeat row).
...
*Last verified against the repo structure on **2026-06-10**. Flag drift via an Issue or correct in a PR.*
