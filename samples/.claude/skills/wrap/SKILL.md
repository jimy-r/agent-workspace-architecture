...
**B4. Registries.** Apply each `Registry rows` change using the lookup table below. **Before touching ANY clone under `public/`, check that checkout's branch** (`git -C <repo> branch --show-current`, `git -C <repo> fetch`, `git -C <repo> rev-list --left-right --count origin/main...HEAD`). Every clone under `public/` is shared with other sessions and is routinely sitting on someone else's branch, behind main. If a clone is not on an up-to-date main, do NOT edit it: report `<repo>: SKIPPED, checkout on <branch>, <n> behind origin/main` as a Discrepancy and let the main thread rebuild the edit against `origin/main` in a worktree. Editing a rolling header or a changelog chain from a stale copy silently deletes whatever entry the stale copy lacks. If `META_ARCHITECTURE.md` is touched: bump its `> **Last updated:** YYYY-MM-DD — <note>` line with the brief's text; then, if a public mirror exists, READ that repo's own `CLAUDE.md` first (it carries the contributor rules), apply the paraphrased change to the public `META_ARCHITECTURE.md`, bump its header with a generic note, run the redaction check that CLAUDE.md specifies, and STOP. Report `Public mirror: edited, redaction check <clean|HITS n>, push pending`. Add the `CHANGELOG entry` at the top of the entries in `<workspace>/CHANGELOG.md` (newest first, below the file header).
