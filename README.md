...
**▶ [Take the interactive tour](https://jimy-r.github.io/agent-workspace-architecture/)** — the clickable five-minute version: the layered architecture, the eleven load-bearing patterns, and one task moving through the system end to end.
...
If you read one thing past this page, read **[PATTERNS.md](PATTERNS.md)** — the eleven load-bearing architectural decisions, each as *problem → pattern → why it beats the obvious alternative → what it costs*. That's where the actual thinking lives.
